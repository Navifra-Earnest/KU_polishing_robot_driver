...
> 리프트(MDROBOT RS485)·배터리(Daly BMS CAN)·조명(Crevis Modbus TCP)은 별도 노드로 구현했습니다.
> 턴테이블(차상모터) 로직은 이 로봇에 해당 없어 제외했습니다.
>
> 전체 ROS 인터페이스/토픽 명세와 고객 검증 절차는 **`docs/INTERFACE.txt`** 참조.
...
└── catkin_ws/
    └── src/
        ...
        │   │   └── lift.launch              # 리프트 드라이버 (RS485)
        │   └── config/
        │       ├── motor_driver.yaml
        │       ├── base_controller.yaml     # 바퀴 반경/윤거/감속비 등 (실측 확정값)
        │       ├── bms_driver.yaml          # BMS CAN 디바이스/폴링 등
        │       └── lift_driver.yaml         # 리프트 포트/속도/폴링 등
        └── crevis_io_driver/            # 조명 LED / 디지털 I/O (Crevis GN-9289, Modbus TCP)
            ├── src/crevis_io_node.cpp       # 순수 소켓 Modbus TCP (외부 라이브러리 의존 0)
            ├── launch/crevis_io.launch
            └── config/crevis_io.yaml        # IP/코일 주소/LED 비트매핑
```
