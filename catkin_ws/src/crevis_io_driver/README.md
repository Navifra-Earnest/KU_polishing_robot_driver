...
폴리싱 로봇 LED 6개와 충전 릴레이를 개별 ON/OFF 하고, Crevis 입력/출력 상태를 읽어
퍼블리시하는 드라이버.
...
```text
/crevis/charging    std_msgs/Bool    true=릴레이 ON, false=릴레이 OFF
```
...
`/crevis/charge_port_on`은 Crevis 출력 image의 bit 6을 다시 읽은 값입니다. 명령 전달
성공 여부뿐 아니라 실제 출력 코일 상태를 이 토픽으로 확인합니다.
...
# 충전 릴레이 켜기/끄기 및 출력 상태 확인
rostopic pub -1 /crevis/charging std_msgs/Bool "data: true"
rostopic echo /crevis/charge_port_on
...
- 충전 릴레이가 안 켜짐 → `/crevis/charge_port_on`과 Y01.06 배선을 확인.
