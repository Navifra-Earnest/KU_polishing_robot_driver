...
void MotorDriverNode::controlLoop(const ros::TimerEvent&)
{
    ...
    // 드라이브 fault 만 자동 리셋 (피드백 타임아웃은 리셋으로 해결 안 되므로 제외)
    if (has_drive_fault && motors_enabled_.load() && !transition_in_progress_.load()) {
        if (controller_->reset_motor()) {
